...
function activate(context) {
    let disposable = vscode.commands.registerCommand('file-template.createFileFromTemplate', async (uri) => {
        ...
        if (template.parameters && template.parameters.length > 0) {
            ...
            const paramHint = template.parameters.map((p) => {
        ...
        if (uris && uris[0]) {
            ...
            try {
                ...
                for (const tpl of imported) {
                    ...
                    if (idx !== -1) {
                        if (!always) {
                            const pick = await vscode.window.showInformationMessage(`已存在同名模板"${tpl.name}"，如何处理？`, { modal: true }, '替换', '跳过', '总是替换', '总是跳过');
                            if (!pick)
                                continue;
                            if (pick === '替换') {
                                templates[idx] = tpl;
